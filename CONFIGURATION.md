...
* `database.write_buffer_size` (type: _integer_, allowed: numbers, default: `16384`) — Maximum size **in KiB** of the database write buffer, after which data gets flushed to disk (ie. `16384` is `16MiB`; to give the size in MiB, multiply it by `1024`, eg. `128 * 1024 = 131072` for `128MiB`)
